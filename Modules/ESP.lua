...
-- Fresh build, compact dev UI, multi-tracer, team colors, player selector
...
function ESP.Init(Client)
	...
	local SelectedPlayer = nil -- nil = ALL
	local PlayerList = {}
	...
	local function NewBundle()
		return {
			TBottom = Draw("Line",{Thickness=1.3,Transparency=1,Visible=false}),
			TTop    = Draw("Line",{Thickness=1.3,Transparency=1,Visible=false}),
			TMid    = Draw("Line",{Thickness=1.3,Transparency=1,Visible=false}),
			Box     = Draw("Square",{Thickness=1.3,Transparency=1,Filled=false,Visible=false}),
			Name    = Draw("Text",{Size=13,Center=true,Outline=true,Font=2,Visible=false}),
			Dist    = Draw("Text",{Size=12,Center=true,Outline=true,Font=2,Visible=false})
		}
	end
	...
	local selectBtn= Button("Target: ALL")
	...
	selectBtn.MouseButton1Click:Connect(function()
		...
		if not SelectedPlayer then
			...
			local idx = table.find(PlayerList, SelectedPlayer)
			if idx and PlayerList[idx + 1] then
				SelectedPlayer = PlayerList[idx + 1]
			else
				SelectedPlayer = nil
			end
		end
